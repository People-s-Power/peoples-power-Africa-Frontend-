			...
			<div className="lg:px-32 my-10">
				...
				<div className="lg:flex mt-6 justify-between">
					<div className="lg:w-[60%]">
						<div className="bg-[#15121280] p-6 rounded-md">
							...
							<div className="flex">
								<div className="p-4 text-center border-r">
									<button onClick={() => setOpen(true)} className="py-2 px-6 my-6 rounded-md bg-white">
										Fund Your wallet
									</button>
									<p className="text-white text-sm">Fund your wallet, this works like a regular account transfer from any source.</p>
								</div>
								<div className="p-4 text-center">
									<button onClick={() => setWithdraw(true)} className="py-2 px-6 my-6  rounded-md bg-white">
										Withdraw funds
									</button>
						...
						<Link href={"/mycamp/profile"} className="p-3 block cursor">
							{user?.bankName ? (
								<div className="p-3">
                  <br />
									...
									<p>{user?.bankName}</p>
                  <span className="text-sm text-[#1bc73f]">Click to change</span>
								</div>
							) : (
					...
					<div className="lg:w-[35%] h-[500px] overflow-y-auto">
						<h5 className="pl-3">My Transactions</h5>
						<div className="flex text-sm p-3 my-4 bg-[#D9D9D929]">
							<p onClick={() => setFilter("All")} className="mx-4 cursor-pointer my-auto">
								All
							</p>
							<p onClick={() => setFilter("Credits")} className="mx-4 cursor-pointer my-auto">
								Credits
							</p>
							<p onClick={() => setFilter("Debits")} className="mx-4 cursor-pointer my-auto">
								Debits
							</p>
						</div>
						<div>
								...
								transactions.map((transaction, index) =>
									filter === "All" ? (
										<div key={index} className="flex justify-between border-b p-3">
											...
											<p className={`${transaction.isInflow ? "text-[#8FBE30]" : "text-[#D80808CC]"} text-sm my-auto`}>
				...
				<Modal open={open} onClose={() => setOpen(false)}>
					...
					<Modal.Body>
						<div className="text-center">
							...
							<button
								disabled={amount <= 0 || undefined}
								className="bg-warning text-white py-2 px-6 rounded-md"
				...
				<Modal open={withdraw} onClose={() => setWithdraw(false)}>
					...
					<Modal.Body>
						<div className="text-center">
							...
							<button className="bg-warning text-white py-2 px-6 rounded-md" onClick={() => withdrawFunds()}>
