				...
				<Modal.Body>
					...
					<div className="my-3 w-full">
						...
						<div className="flex flex-col gap-2">
							...
							<button className="bg-warning text-white rounded px-3 py-1" onClick={() => setSubtasks((prev) => [...prev, ""])}>
								Add more subtasks
							</button>
						</div>
					</div>
              ...
              options={operators} /> */}
						<select onChange={(e) => setAssign([e.target.value])} className="w-full p-2 rounded-md">
							<option value="" className="hidden">
								{operators.length > 0 ? "Select an admin" : "Add admins assign task to"}
							</option>
							...
							{previewImages.map((image, index) =>
								image?.type === "image" ? (
									<div className="w-[100px] h-[100px] m-[3px]" key={index}>
										<img src={image?.url} alt={`Preview ${index}`} className=" object-cover w-full h-full" />
										<div className="flex  cursor-pointer text-[red] justify-center items-center" onClick={() => handleDelSelected(index)}>
											Delete
										</div>
									</div>
								) : (
									<div className="w-[100px] h-[100px] m-[3px]" key={index}>
										<video src={image?.url} width="500" autoPlay muted className="embed-responsive-item w-full object-cover h-full">
